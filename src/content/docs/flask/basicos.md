...
Una aplicación [Flask](https://lineadecodigo.com/flask/) es un objeto que recibe peticiones HTTP y decide qué función debe ejecutarse para cada URL. La asociación entre una URL y una función se llama **ruta**. El valor devuelto por esa función se convierte en la respuesta enviada al navegador o al cliente de una API.


## Características de Básicos Flask

- **Instalación aislada.** Es recomendable crear un entorno virtual para que [Flask](https://lineadecodigo.com/flask/) y sus dependencias no interfieran con otros proyectos de Python.
...
- **Respuestas flexibles.** Una vista puede devolver texto, [HTML](https://lineadecodigo.com/html/), un diccionario serializado como JSON o una tupla con el cuerpo y el código de estado HTTP.
...
- **Servidor de desarrollo.** [Flask](https://lineadecodigo.com/flask/) incluye un servidor para pruebas locales con recarga automática. No debe utilizarse como servidor público de producción.
...
Primero se crea y activa un entorno virtual y se instala [Flask](https://lineadecodigo.com/flask/):
...
Al abrir `http://127.0.0.1:5000/api/saludo?nombre=Victor`, Flask devuelve una respuesta JSON equivalente a:
